...
Then, we fill out our matrix with `$0$`s and `$1$`s, with `$1$` signifying that an edge connects the vertice in its column and its row. So, for example, in the top right empty box, this would
